...
var current_speed = move_speed * GameManager.speed_multiplier
...
@onready var player_sprite : AnimationPlayer = $Visual/Grim/AnimationPlayer
@onready var player_node = $Visual
...
func movement():
	# Gravity
	if !is_on_floor():
		velocity.y += gravity * GameManager.gravity_multiplier
	elif is_on_floor():
	...
	if movement_enabled:
		if Input.is_action_pressed("Left"):
			velocity.x = -move_speed * GameManager.speed_multiplier
...
func check_spike():
	var feet_pos = global_position + Vector2(0, 28)
	var cell = ground.local_to_map(feet_pos)
	var tile_data = ground.get_cell_tile_data(cell)

	if tile_data and tile_data.get_custom_data("is_spike") and can_damage:
		GameManager.hp = max(GameManager.hp - 20, 0)
		damage_tween()
